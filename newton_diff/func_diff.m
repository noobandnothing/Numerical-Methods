...
% Append  y's delta %
    ...
    for y = 2 : num_point
       ...
       while index < (rows-counter+1)
            myTab((2*index+2)/2,y+1) =  (myTab(index+2,y) - myTab(index,y));
            index =  index +2 ;
       end
       counter = counter +1;
    end
...
function eqn = GetEquation(num_point,myTab,flag)
    ...
    % forward
       ...
       for index = 2: ceil((num_point*2 -1)/2)
           ...
           if index >= 3
              for counter = 3 : index
                   cetav = cetav * (CETA-counter+2);
              end
           end
           cetav = cetav / factorial(int32(index-1));
           res = res * cetav;
           eqn = eqn + res;
       end
    %}
